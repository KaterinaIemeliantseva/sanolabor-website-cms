        ...
        <form action="#" data-c="<?php echo $handler->getClass($foo); ?>" data-m="update" method="post" class="edit_form_validate form-group">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <?php $handler->html_checkbox(['label' => 'Upoštevaj datum od do', 'name' => 'upostevaj', 'status' => (!empty($data->upostevaj))]); ?>
                    <?php $handler->html_input(['label' => 'Naziv', 'name' => 'naziv', 'value' => (!empty($data->naziv)) ? $data->naziv : '', 'required' => true]); ?>
                    <?php $handler->html_single_file_upload(['label' => 'PDF dokument', 'name' => 'dokument', 'value' => (!empty($data->dokument)) ? $data->dokument : '', 'required' => false]); ?>                    
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
