...
class Database
{
	...
	{
		$stmt = $this->link_id->prepare("SELECT * FROM cms_user WHERE username = :username LIMIT 1");
		$stmt->bindValue(':username', $username, PDO::PARAM_STR);
		$stmt->execute();
		return $stmt->fetch(PDO::FETCH_ASSOC);
	}

	public function storeRememberToken($userID, $tokenHash)
	{
		$stmt = $this->link_id->prepare("UPDATE cms_user SET remember_token = :token WHERE id = :id");
	...
	{
		$stmt = $this->link_id->prepare("SELECT * FROM cms_user WHERE remember_token = :token LIMIT 1");
		$stmt->bindValue(':token', $tokenHash, PDO::PARAM_STR);
		$stmt->execute();
		return $stmt->fetch(PDO::FETCH_ASSOC);
	}
	...
	{
		$stmt = $this->link_id->prepare("UPDATE cms_user SET remember_token = NULL WHERE id = :id");
		$stmt->bindValue(':id', $userID, PDO::PARAM_INT);
		$stmt->execute();
	}
	...
	{
		$stmt = $this->link_id->prepare("UPDATE cms_user SET password = :password WHERE id = :id");
		$stmt->bindValue(':password', $newHash, PDO::PARAM_STR);
		$stmt->bindValue(':id', $userID, PDO::PARAM_INT);
		$stmt->execute();
	}
